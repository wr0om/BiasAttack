...
from transformers import (
    ...
    GPTNeoXForCausalLM,
    LlamaForCausalLM,
    MistralForCausalLM,     # already referenced later
    Qwen2ForCausalLM,       # Qwen-2 & 2.5  :contentReference[oaicite:0]{index=0}
    Phi3ForCausalLM,        # Phi-3 / Phi-4 share the same class  :contentReference[oaicite:2]{index=2}
    GemmaForCausalLM,        # Gemma-3  :contentReference[oaicite:3]{index=3}
    PreTrainedModel,
)
...
def get_embedding_layer(model: PreTrainedModel):
    """Return the token-embedding layer for *any* HF causal-LM."""
    # ---- Fast paths you already had ---------------------------------------
    ...
    elif getattr(model.config, "model_type", "") == "falcon":
        return model.transformer.word_embeddings

    # ---- Generic fallback (covers Qwen-2, DeepSeek, Phi-3/4, Gemma-3, …) --
    try:
        return model.get_input_embeddings()           # universal API :contentReference[oaicite:2]{index=2}
    except AttributeError:
        raise ValueError(f"Cannot locate embedding layer for {type(model)}")


def get_embedding_matrix(model: PreTrainedModel):
    """Return the weight matrix behind the embedding layer."""
    return get_embedding_layer(model).weight


def get_embeddings(model: PreTrainedModel, input_ids):
    """Return the *embeddings* (not logits) for a tensor of token IDs."""
